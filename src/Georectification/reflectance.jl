...
function generateReflectance2!(Data, specpath, spechdr, λs)
    # load in downwelling spectrum
    spectrum, hspec, pspec = read_envi_file(specpath, spechdr);
    spec = @view spectrum[:,1,1]
    ...
    Threads.@threads for λ ∈ 1:length(λs)
        @inbounds Data[λ,:,:] .= clamp.(π .* Data[λ,:,:] ./ adjustedSpec[λ], 0.0, 1.0)  # reflectance is between 0 and 1
    end
end
